...
from utils import l2_dist, get_group_activations_from_layer, binary_search, \
    get_layer_result_for_image_batch, get_layer_result_for_image_batch_torch, \
    get_partition_id_by_image_id, get_image_ids_by_partition_id, \
...
def get_group_sample(dataset, image_sample_id, layer_id, model, neuron_group):
    if is_torch:
        import torch
        layer_result_sample = model.get_layer_result_by_layer_id(torch.stack([dataset[image_sample_id]]), layer_id)[0]
...
def run_nn_and_update_things(dataset, group_activation_cached, group_sample, heap, image_batch, k, layer_id, model,
    ...
    if is_torch:
        layer_result = get_layer_result_for_image_batch_torch(model, dataset, image_batch, layer_id, BATCH_SIZE)
    else:
