...
extern "C" {
                        ...
                        float* par_lower_bound, float* par_upper_bound)
    {
        ...
        int width_partition = int((idx_act[cutoff].value - idx_act[n_images - 1].value) / (n_partitions - 1)) + 1;
        ...
        for (int i = 0; i < n_images; ++i)
        {
            ...
            {
                partition = 1 + int(idx_act[cutoff].value - idx_act[i].value) / width_partition;
